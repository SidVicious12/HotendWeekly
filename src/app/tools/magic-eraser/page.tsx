...
export default function MagicEraserPage() {
  ...
  const handleMagicErase = async () => {
    ...
    if (!prompt.trim()) {
      setError('Please describe what to remove');
      return;
    }
    ...
    try {
      ...
      formData.append('prompt', `remove ${prompt}, fill background seamlessly`);
      ...
      if (!response.ok) throw new Error(data.error || 'Magic Eraser failed');
      ...
      <section className="pt-20 pb-12 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-xl md:text-2xl text-gray-600 max-w-4xl mx-auto">
            Remove unwanted objects instantly with AI. Just describe what to remove.
          </p>
        </div>
      </section>
              ...
              <div className="space-y-8">
                  ...
                  <div>
                    ...
                    {processedImage ? (
                      ...
                      <div className="bg-gray-100 rounded-2xl h-64 flex items-center justify-center text-gray-400">
                        {isProcessing ? 'Removing object...' : 'Result will appear here'}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                  ...
                  <div className="flex gap-4">
                    <input
                      ...
                      placeholder="e.g., the red chair, the text watermark, the person in background"
                      className="flex-1 rounded-xl border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 p-4"
                    />
                    <button
                      onClick={handleMagicErase}
                      disabled={isProcessing || !prompt}
                      className="bg-purple-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-purple-700 disabled:opacity-50 transition-colors"
                    >
                ...
                {processedImage && (
                  <div className="flex gap-4 justify-end">
                    ...
                    <button onClick={() => { setUploadedImage(null); setProcessedImage(null); setPrompt(''); }} className="bg-gray-600 text-white px-6 py-3 rounded-xl hover:bg-gray-700">
                      New Image
                    </button>
                  </div>
                )}
