...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const limitCheck = await checkUsageLimit(user.id, 'layer-detail-enhancer')
        ...
        // Using InstructPix2Pix to smooth layer lines specifically
        const prompt = "smooth out the visible 3d print layer lines, make surface smooth and polished like injection molded plastic, keep details"
        ...
        await incrementUsage(user.id, {
            toolName: 'layer-detail-enhancer',
        ...
        if (userId) await incrementUsage(userId, { toolName: 'layer-detail-enhancer', toolCategory: 'ai_tools', status: 'error', processingTimeMs: Date.now() - startTime })
